...
public class ModelAnalyzer {
		...
		if (managedByStereotype != null) {
			...
			for (int j = 0; j < tags.size(); ++j) {
				...
				if (value.size() > 0) {
					...
					case "role":
						role = (Class) value.get(0);
						managedByRole = role.getName();
						break;
		...
		if (uiClassStereotype != null) {
			...
			for (int j = 0; j < tags.size(); ++j) {
				...
				if (value.size() > 0) {
					...
					case "add":
						add = (Boolean) value.get(0);
						break;
					case "view":
						view = (Boolean) value.get(0);
						break;
					case "update":
			...
			UIClass uiClass = new UIClass(label, add, view, update, delete, groupPresType, singlePresType,
					presPropertyName);

			fmClass.setUiClass(uiClass);

			System.out.println(
					"UIClass(label, add, view, update, delete, groupPresType, singlePresType, presPropertyName): "
							+ label + " " + add + " " + view + " " + update + " " + delete + " " + groupPresType + " "
							+ singlePresType + " " + presPropertyName);
		}
	...
	private FMProperty getPropertyData(Property p, Class cl) throws AnalyzeException {
		...
		FMProperty fmProperty = new FMProperty(attName, typeName, p.getVisibility().toString(), lower, upper,
		...
		if (textValPropStereotype != null) {
			...
			System.out.println("TextValProperty(unique, notNull, minLength, maxLength): " + unique + " " + notNull + " "
					+ minLength + " " + maxLength);
		}
		...
		if (numValPropStereotype != null) {
			...
			System.out.println("NumValProperty(unique, notNull, minValue, maxValue): " + unique + " " + notNull + " "
					+ minValue + " " + maxValue);
		}
