...
public class SpringGenerator extends BasicGenerator {
	 ...
	 * LinkedList<FMClass> ret = new LinkedList<>(); for (Iterator<FMClass> iterator
	 * = classes.iterator(); iterator.hasNext();) { FMClass cl = iterator.next();
	 * if(!ret.contains(cl)) { iterator.remove(); if(hasAssociationList(cl)) {
	 * System.out.println("has association"); ret.addAll(orderClasses(classes)); }
	 * System.out.println("doesn't have association"); ret.add(cl); } } return ret;
	 * }
	 * 
	 * public boolean hasAssociationList(FMClass cl) { for(FMProperty property :
	 * cl.getProperties()) { if(property.getUpper() == -1 &&
	 * property.getAssociation() == true) { return true; } } return false; }
	 */
